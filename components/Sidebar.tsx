...
import { 
  ...
  ChevronLeft, ChevronRight, Truck, Box, Layers, Wallet, Users
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ user, currentPage, setCurrentPage, isOpen, toggleSidebar, settings }) => {
  ...
  return (
    <aside 
      className={`fixed lg:static inset-y-0 left-0 z-50 flex flex-col bg-[#0f172a] text-slate-300 transition-all duration-300 shadow-2xl lg:shadow-none border-r border-slate-800 ${
                ...
                {visibleItems.map(item => {
                  ...
                  return (
                    <button 
                      ...
                      className={`w-full flex items-center ${isOpen ? 'px-4 py-3' : 'justify-center py-3'} rounded-lg transition-all ${
                        active 
