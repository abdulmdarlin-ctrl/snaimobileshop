   ...
   return (
      <div className="space-y-6 animate-in pb-20 font-sans">
         ...
         <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 no-print">
            <div>
               <h1 className="text-2xl font-black text-slate-900 italic uppercase tracking-tight">Procurement</h1>
               <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">Supply Chain Management</p>
            </div>
            ...
            <div className="flex gap-3">
               <div className="hidden md:flex gap-4 mr-6">
                  ...
                  <div className="flex items-center gap-3 px-4 py-2 bg-white rounded-xl border border-slate-100 shadow-sm">
                     ...
                     <div>
                        ...
                        <p className="text-sm font-black text-slate-900">{stats.vendorCount}</p>
                     </div>
                  </div>
               </div>
               ...
               <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-slate-900/20 hover:bg-black transition-all active:scale-95"
               >
         ...
         <div className="bg-white p-2 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 no-print">
            <div className="flex p-1 bg-slate-100 rounded-xl w-full md:w-auto">
               <button
                  ...
                  className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${viewMode === 'orders' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
               >
                  <ClipboardList size={14} /> Purchase Orders
               </button>
               <button
                  onClick={() => setViewMode('vendors')}
                  className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${viewMode === 'vendors' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
               >
                     ...
                     <div className="overflow-x-auto">
                        <table className="w-full text-left">
                           <thead className="bg-slate-50 border-b border-slate-100">
                              <tr>
                                 <th className="pl-6 py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Date</th>
                                 <th className="py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Invoice Ref</th>
                                 <th className="py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Supplier</th>
                                 <th className="text-center py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Items</th>
                                 <th className="text-right py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Total Value</th>
                                 <th className="text-center py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Status</th>
                                 <th className="pr-6 text-right py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Action</th>
                              </tr>
                           </thead>
                           <tbody className="divide-y divide-slate-50">
                              {filteredOrders.map(order => (
                                 <tr key={order.id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="text-center py-4">
                                       <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wide bg-emerald-50 text-emerald-600 border border-emerald-100">
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                  {filteredVendors.map(vendor => (
                     <div key={vendor.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group relative">
                        ...
                        <h3 className="text-sm font-black text-slate-900 uppercase truncate">{vendor.name}</h3>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4">{vendor.contactPerson}</p>
         ...
         {isOrderModalOpen && (
            <div className="fixed inset-0 bg-slate-900/60 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm animate-in">
               <div className="bg-white w-full max-w-5xl rounded-[2rem] shadow-2xl overflow-hidden border border-white/20 flex flex-col max-h-[95vh]">
                  {/* Modal Header */}
                  <div className="p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           <h2 className="text-lg font-black text-slate-900 uppercase italic tracking-tight">Receive Stock</h2>
                           <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">New Purchase Order</p>
                        </div>
                     </div>
                     <button onClick={() => setIsOrderModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-900 rounded-xl hover:bg-slate-100 transition-all"><X size={20} /></button>
                  </div>
                        ...
                        <section className="space-y-4">
                           ...
                           <div className="space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
                              ...
                              <button
                                 ...
                                 className="w-full py-3 bg-slate-900 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all disabled:opacity-50"
                              >
                                 Add to List
                              </button>
                           </div>
            ...
            <div className="fixed inset-0 bg-slate-900/60 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm animate-in">
               <div className="bg-white w-full max-w-lg rounded-[2rem] shadow-2xl p-8 border border-white/20">
                  <div className="flex justify-between items-center mb-6">
                     <h2 className="text-xl font-black text-slate-900 uppercase italic tracking-tighter">{isEditingVendor ? 'Edit Vendor' : 'New Vendor'}</h2>
                     <button onClick={() => setIsVendorModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-900 rounded-xl hover:bg-slate-50"><X size={24} /></button>
                  </div>
         ...
         {selectedOrder && (
            <div className="fixed inset-0 bg-slate-900/60 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm animate-in">
               <div className="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden border border-white/20 max-h-[85vh] flex flex-col">
                  <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                     <div>
                        <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">Order Details</h2>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{selectedOrder.invoiceNo}</p>
                     </div>
                     <button onClick={() => setSelectedOrder(null)} className="p-2 text-slate-400 hover:text-slate-900 rounded-xl hover:bg-slate-100"><X size={20} /></button>
                  </div>
