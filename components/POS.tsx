...
const POS: React.FC<POSProps> = ({ user }) => {
   ...
   const renderCatalog = () => (
      <div className="flex-1 flex flex-col min-w-0 bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden h-full">
         ...
         <div className="p-6 border-b border-slate-100 space-y-4 bg-white sticky top-0 z-10">
            <div className="flex gap-4">
               <div className="relative flex-1 group">
                  ...
                  <input
                     ref={searchInputRef}
                     className="w-full pl-12 pr-12 h-14 bg-slate-50 rounded-2xl text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-slate-100 focus:bg-white transition-all border border-transparent focus:border-slate-200"
               ...
               <button
                  ...
                  className="relative p-4 bg-slate-900 text-white rounded-2xl hover:bg-black transition-all shadow-lg shadow-slate-900/20 active:scale-95"
                  title="Held Sales"
               >
                  <Pause size={24} />
                  {heldSales.length > 0 && (
                     <span className="absolute -top-2 -right-2 w-6 h-6 bg-rose-500 text-white text-[10px] font-black flex items-center justify-center rounded-full border-2 border-white animate-bounce">
                        {heldSales.length}
                     </span>
                  )}
               </button>
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
               <div className="flex items-center gap-2 px-3 py-2 bg-slate-100 rounded-xl text-slate-400 shrink-0">
                  <Filter size={14} />
         ...
         <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30">
            {loading ? (
               ...
               <div className="h-full flex flex-col items-center justify-center text-slate-300">
                  ...
                  <p className="text-xs font-medium text-slate-400 mt-2">Try a different search or category</p>
               </div>
            ) : (
                  ...
                  {filteredProducts.map(product => {
                     ...
                     return (
                        <button
                           ...
                           className="bg-white p-5 rounded-[2rem] border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all text-left group flex flex-col justify-between h-[220px] relative overflow-hidden active:scale-95"
                        >
                           {isOut && (
                              <div className="absolute inset-0 bg-white/80 z-10 flex items-center justify-center backdrop-blur-[2px]">
                                 <span className="bg-rose-600 text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg">Sold Out</span>
                              </div>
                           )}
                           ...
                           <div>
                              <div className="flex justify-between items-start mb-4">
                                 ...
                                 <div className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter border ${isOut ? 'bg-rose-50 text-rose-600 border-rose-100' :
                              ...
                              <h3 className="text-sm font-black text-slate-900 line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">{product.name}</h3>
                              <div className="flex items-center gap-1.5 mt-2">
                                 <Tag size={10} className="text-slate-300" />
                                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate">
                                    {product.brand || 'General'} • {product.sku}
                                 </p>
                              </div>
                           </div>
                           ...
                           <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between">
                              <div>
                                 <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1">Price</p>
                                 <p className="text-lg font-black text-slate-900">
                                    <span className="text-xs text-slate-400 mr-0.5 font-medium">UGX</span>
                                    {product.selling_price.toLocaleString()}
                                 </p>
                              </div>
                              <div className="w-10 h-10 rounded-2xl bg-slate-900 text-white flex items-center justify-center shadow-lg shadow-slate-900/20 group-hover:scale-110 transition-transform">
   ...
   return (
      <div className="flex flex-col lg:flex-row h-full gap-6 font-sans pb-20 lg:pb-0 animate-in fade-in duration-500">
         ...
         {/* RIGHT: Cart & Checkout */}
         <div className="w-full lg:w-[400px] xl:w-[450px] bg-white rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col overflow-hidden shrink-0 h-full">
            {/* Cart Header */}
               ...
               <div className="flex items-center gap-3">
                  ...
                  <div>
                     ...
                     <div className="flex items-center gap-2">
                        ...
                        <span className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter border ${pricingMode === 'Retail' ? 'bg-blue-50 text-blue-600 border-blue-100' :
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/50">
               {cart.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-slate-300">
                     ...
                     <p className="text-[10px] font-medium text-slate-400 mt-2">Add items to begin transaction</p>
                  </div>
               ) : (
                  cart.map((item, idx) => (
            ...
            <div className="p-6 bg-white border-t border-slate-100 shadow-[0_-4px_20px_rgba(0,0,0,0.02)] z-10 space-y-5">
               <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Pricing: {pricingMode}</span>
               </div>
                  ...
                  <div className="p-8 space-y-8">
                     ...
                     <section className="space-y-4">
                        ...
                        <div className="space-y-3">
                           <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide ml-1">Global Discount (%)</label>
            ...
            <div className="fixed inset-0 bg-slate-900/60 z-[10000] flex items-center justify-center p-4 backdrop-blur-sm animate-in">
               <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/20">
                  <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                     <div>
                        <h2 className="text-lg font-black text-slate-900 uppercase italic tracking-tight">Confirm Transaction</h2>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Final Check</p>
                     </div>
                     <button onClick={() => setIsConfirmSaleModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-900 rounded-xl hover:bg-slate-100 transition-all"><X size={20} /></button>
                  </div>
                  <div className="p-8 space-y-6">
                     ...
                     <div className="grid grid-cols-2 gap-3">
                        <button onClick={() => setIsConfirmSaleModalOpen(false)} disabled={processingPayment} className="py-4 bg-slate-100 text-slate-600 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all">Cancel</button>
                        <button onClick={processPayment} disabled={processingPayment} className="py-4 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-emerald-600/20 hover:bg-emerald-700 transition-all flex items-center justify-center gap-2">
                     ...
                     <div className="flex-1 overflow-y-auto p-4 bg-slate-200 flex justify-center">
                        <div
                           ...
                           <div className="mb-2">
                              ...
                              {settings?.receiptShowCashier !== false && <div>Cashier: {lastSale.cashierName}</div>}
                           ...
                           <div className="space-y-1 mb-2">
                              {(() => {
                                 const grossSubtotalBeforeAnyDiscounts = lastSale.items.reduce((acc, item) => acc + ((item.originalPrice || item.price) * item.quantity), 0);
                                 const itemLevelDiscountAmount = lastSale.items.reduce((acc, item) => acc + ((item.originalPrice || item.price) - item.price) * item.quantity, 0);
                                 // lastSale.subtotal is the subtotal after item-level discounts
                                 const globalDiscountAmountInReceipt = (lastSale.subtotal * (lastSale.globalDiscountPercentage || 0)) / 100;
                                 const netSubtotalAfterAllDiscounts = lastSale.subtotal - globalDiscountAmountInReceipt;
                                 ...
                                 return (
                                    <>
                                       ...
                                       {lastSale.customerType === 'Retail' && globalDiscountAmountInReceipt > 0 && (
                                          <div className="flex justify-between">
                                             <span>Global Discount ({lastSale.globalDiscountPercentage}%)</span>
                              ...
                              {settings?.receiptShowTaxDetail !== false && lastSale.tax > 0 && (
                                 <div className="flex justify-between">
                                    <span>TAX ({settings?.taxPercentage || 18}%)</span>
                                    <span>{lastSale.tax.toLocaleString()}</span>
                                 </div>
                              )}
                              <div className="flex justify-between font-bold text-sm border-t border-black pt-1 mt-1"> {/* This border is now redundant if NET SUBTOTAL has one */}
                                 <span>TOTAL</span>
            ...
            isSelectPricingModalOpen && (
               <div className="fixed inset-0 bg-slate-900/60 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm animate-in">
                  <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/20">
                     <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <div>
                           <h2 className="text-lg font-black text-slate-900 uppercase italic tracking-tight">Select Pricing</h2>
                           <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Choose Customer Type</p>
                        </div>
                        <button onClick={() => setIsSelectPricingModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-900 rounded-xl hover:bg-slate-100 transition-all"><X size={20} /></button>
                     </div>

                     <div className="p-8 space-y-4">
                        <div className="space-y-3">
                           <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide ml-1">Customer Type</label>
                           <div className="relative">
                              <select
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 p-6">
                           <div className="bg-slate-900 p-6 rounded-3xl text-white shadow-2xl shadow-slate-900/20 relative overflow-hidden group">
                              ...
                              <p className="text-4xl font-black tracking-tighter">
                                 <span className="text-xl text-slate-500 mr-1 font-medium">UGX</span>
                           ...
                           <div className={`p-6 rounded-3xl text-white shadow-2xl group ${reportData.totals.profit >= 0 ? 'bg-emerald-600 shadow-emerald-600/20' : 'bg-red-600 shadow-red-600/20'}`}>
                              ...
                              <p className="text-4xl font-black tracking-tighter">
                                 <span className="text-xl text-white/80 mr-1 font-medium">UGX</span>
                        ...
                        <div id="sales-history-table" className="w-full bg-white h-full">
                           <table className="w-full text-left border-collapse">
                              <thead className="bg-slate-50/80 backdrop-blur-sm sticky top-0 z-10">
                                 <tr>
                                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">Receipt #</th>
                                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">Date & Time</th>
                                    ...
                                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right">Profit</th>
                                    ...
                                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right no-print">Action</th>
                                 </tr>
                              </thead>
                              <tbody>
                                 {(() => {
                                             ...
                                             <tr className="hover:bg-slate-50/50 transition-colors group border-b border-slate-100">
                                                ...
                                                <td className="px-6 py-4">
                                                   <div className="flex flex-col">
                                                      ...
                                                      <span className="text-[10px] text-slate-400">{typeof sale.customerType === 'string' ? sale.customerType : 'Retail'}</span>
                                                   </div>
                                                </td>
                  ...
                  <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden border border-white/20">
                     <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                        <div className="flex items-center gap-3">
                           ...
                           <div>
                              <h2 className="text-lg font-black text-slate-900 uppercase italic tracking-tight">Hold Sale</h2>
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Add a reference note</p>
                           </div>
                        </div>
                        <button onClick={() => setIsHoldModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-900 rounded-xl hover:bg-slate-100 transition-all"><X size={20} /></button>
                     </div>
                     <div className="p-6 space-y-4">
                        <div className="space-y-2">
                           <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide ml-1">Note / Description</label>
                           <textarea
                              autoFocus
                              className="w-full h-24 bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm font-medium outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all resize-none"
                        ...
                        <div className="grid grid-cols-2 gap-3">
                           <button onClick={() => setIsHoldModalOpen(false)} className="py-3 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all">Cancel</button>
                           <button
                              onClick={confirmHoldSale}
                              className="py-3 bg-amber-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-amber-500/20 hover:bg-amber-600 transition-all active:scale-95"
                           >
                              Confirm Hold
                           </button>
                        </div>
         ...
         {/* --- HELD SALES MODAL --- */}
         {
            isHeldSalesModalOpen && (
               <div className="fixed inset-0 bg-slate-900/60 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm animate-in">
                  <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden border border-white/20 flex flex-col max-h-[80vh]">
                     <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                        <div>
                           <h2 className="text-lg font-black text-slate-900 uppercase italic tracking-tight">Held Sales</h2>
                           <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Resume Pending Orders</p>
                        </div>
                        <button onClick={() => setIsHeldSalesModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-900 rounded-xl hover:bg-slate-100 transition-all"><X size={20} /></button>
                     </div>
                              ...
                              <div key={s.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between group hover:border-amber-200 transition-all">
                                 <div className="min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                       ...
                                       <span className="text-[9px] font-bold text-slate-400 uppercase bg-slate-100 px-1.5 py-0.5 rounded">{s.items.length} Items</span>
                                    </div>
                                    <p className="text-[10px] text-slate-400 font-medium">
                                       {new Date(s.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} • UGX {s.total.toLocaleString()}
                                    </p>
                                    {s.notes && (
                                       <p className="text-[10px] text-amber-600 font-bold mt-1 italic truncate max-w-[200px]">Note: {s.notes}</p>
                                    )}
                                 </div>
                                 <div className="flex items-center gap-2">
